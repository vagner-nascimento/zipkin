...
function loadControllers(pattern, opts) {
    var router = express_1.Router();
    awilix_router_core_1.findControllers(pattern, tslib_1.__assign({}, opts, { absolute: true })).forEach(_registerController.bind(null, router));
    return router;
}
...
function _registerController(router, stateAndTarget) {
    ...
    rolledUp.forEach(function (methodCfg, methodName) {
        methodCfg.verbs.forEach(function (httpVerb) {
            ...
            ;
            (_a = router)[method].apply(_a, [methodCfg.paths].concat(methodCfg.beforeMiddleware, [
